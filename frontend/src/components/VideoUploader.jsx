...
//
// Admin-only component for uploading course videos.
//
// Upload flow:
...
//
// Why XHR instead of fetch: XHR supports upload progress events natively.
// Why direct to S3: Videos are 1–2GB. Routing through Express would be slow
// and memory-intensive. S3 handles it natively.
...
// Direct XHR upload to S3 presigned URL with progress tracking
...
export default function VideoUploader({ courseId, onUploadComplete }) {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get("/upload/presigned-url", {
        params: {
          fileType: file.type,
          courseId,
          fileName: file.name,
      ...
      // Step 3: Notify parent with the S3 key to save in DB
      setStatus("done");
  ...
  return (
    <div className="space-y-3">
      ...
      {status === "idle" && (
        <button
          ...
          className="flex items-center gap-2 border-2 border-dashed border-gray-300
                     hover:border-indigo-400 rounded-lg px-4 py-3 text-sm
                     text-gray-600 hover:text-indigo-600 transition-colors w-full
                     justify-center"
        >
      ...
      {status === "uploading" && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-500">
            Uploading directly to storage... do not close this tab.
          </p>
        </div>
      )}
      ...
      {/* Error state */}
      {status === "error" && (
        <div className="flex items-center justify-between bg-red-50 border
